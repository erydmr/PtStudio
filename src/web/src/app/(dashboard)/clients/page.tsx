        ...
        <TableBody>
          {clients.map((c) => (
            <TableRow key={c.id}>
              ...
              <TableCell><Badge variant={c.isActive ? "default" : "destructive"}>{c.isActive ? "Aktif" : "Pasif"}</Badge></TableCell>
            </TableRow>
          ))}
          {clients.length === 0 && (
            <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground">Musteri bulunamadi.</TableCell></TableRow>
          )}
        </TableBody>
      </Table>
    </div>
